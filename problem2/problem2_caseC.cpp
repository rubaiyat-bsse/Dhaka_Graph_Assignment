...
int findNearestRoadNode(Graph& G, double lon, double lat) {
    ...
    for(int i = 0; i < G.nodes.size(); i++) {
        if(G.nodes[i].type != "road") continue; // Only consider road nodes
...
int main(){
    ...
    if(srcNode == -1) {
        cerr << "Error: Could not find nearest road node for source!" << endl;
        return 1;
    }
    
    if(destNode == -1) {
        cerr << "Error: Could not find nearest road node for destination!" << endl;
        return 1;
    }
    
    double srcDist = haversine(G.nodes[srcNode].longitude, G.nodes[srcNode].latitude, srcLon, srcLat);
    double destDist = haversine(G.nodes[destNode].longitude, G.nodes[destNode].latitude, destLon, destLat);
    
    cout << "Source: Random point (" << srcLon << ", " << srcLat << ")" << endl;
    cout << "  Nearest road node: " << srcNode 
         << " at (" << G.nodes[srcNode].longitude << ", " << G.nodes[srcNode].latitude << ")" << endl;
    cout << "  Distance to nearest node: " << fixed << setprecision(3) << srcDist << " km" << endl;
    cout << endl;
    
    cout << "Destination: Random point (" << destLon << ", " << destLat << ")" << endl;
    cout << "  Nearest road node: " << destNode 
         << " at (" << G.nodes[destNode].longitude << ", " << G.nodes[destNode].latitude << ")" << endl;
    cout << "  Distance to nearest node: " << fixed << setprecision(3) << destDist << " km" << endl;
    cout << endl;
    ...
    // Add walking costs
    double walkingCostSrc = srcDist * 20.0;  // Walk to nearest road node
    double walkingCostDest = destDist * 20.0; // Walk from nearest road node
    double totalMinCost = dist[destNode] + walkingCostSrc + walkingCostDest;
    
    cout << "Walking cost to source node: ৳" << fixed << setprecision(2) << walkingCostSrc << endl;
    cout << "Path cost (on network): ৳" << fixed << setprecision(2) << dist[destNode] << endl;
    cout << "Walking cost from destination node: ৳" << fixed << setprecision(2) << walkingCostDest << endl;
    cout << "Minimum total cost: ৳" << fixed << setprecision(2) << totalMinCost << endl;
    cout << endl;
    ...
    cout << "\nKML file generated: route_caseC.kml" << endl;
