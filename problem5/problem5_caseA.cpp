...
void printSegmentedPath(Graph& G, vector<int>& path, double srcLon, double srcLat, 
    ...
    cout << "Total travel time: " << (int)dist[path.back()] << " minutes\n";
    cout << "Arrival time: " << formatTime(arrivalTime[path.back()]) << "\n";
}
...
int main(){
    ...
    if(G.coordToId.find(srcCoord) == G.coordToId.end()){
        cout << "Error: Source coordinate not found in graph!\n";
        return 1;
    }
    
    if(G.coordToId.find(destCoord) == G.coordToId.end()){
        cout << "Error: Destination coordinate not found in graph!\n";
        return 1;
    }
    ...
    cout << "KML file generated: route_caseA.kml\n";
