...
EdgeMatch findEdgeForPoint(Graph& G, double lon, double lat) {
    ...
    for(int u = 0; u < G.V; u++) {
        for(auto& [v, edgeWeight] : G.adj[u]) {
            if(u < v) {
                double distUV = edgeWeight;
                double distUP = haversine(G.nodes[u].longitude, 
                                         G.nodes[u].latitude, 
                                         lon, lat);
...
void DijkstraFromEdge(Graph& G, EdgeMatch src) {
    ...
    dist.assign(G.V, DBL_MAX);
    parent.assign(G.V, -1);
    
    dist[src.node1] = src.distToNode1;
    dist[src.node2] = src.distToNode2;
...
int main() {
    ...
    EdgeMatch srcEdge = findEdgeForPoint(G, src_lon, src_lat);
    EdgeMatch destEdge = findEdgeForPoint(G, dest_lon, dest_lat);
    
    if(!srcEdge.found) {
        cerr << "Error: source coordinate not on any edge" << endl;
        return 1;
    }
    
    if(!destEdge.found) {
        cerr << "Error: destination coordinate not on any edge" << endl;
        return 1;
    }
    
    cout << "Source: (" << fixed << setprecision(6) << src_lon << ", " << src_lat << ")" << endl;
    cout << "On edge between nodes " << srcEdge.node1 << " and " << srcEdge.node2 << endl;
    cout << "  Distance to node " << srcEdge.node1 << ": " << fixed << setprecision(3) 
         << srcEdge.distToNode1 << " km" << endl;
    cout << "  Distance to node " << srcEdge.node2 << ": " << fixed << setprecision(3) 
         << srcEdge.distToNode2 << " km" << endl;
    cout << endl;
    
    cout << "Destination: (" << fixed << setprecision(6) << dest_lon << ", " << dest_lat << ")" << endl;
    cout << "On edge between nodes " << destEdge.node1 << " and " << destEdge.node2 << endl;
    cout << "  Distance to node " << destEdge.node1 << ": " << fixed << setprecision(3) 
         << destEdge.distToNode1 << " km" << endl;
    cout << "  Distance to node " << destEdge.node2 << ": " << fixed << setprecision(3) 
         << destEdge.distToNode2 << " km" << endl;
    cout << endl;
    
    DijkstraFromEdge(G, srcEdge);
    
    double dist1 = dist[destEdge.node1] + destEdge.distToNode1;
    double dist2 = dist[destEdge.node2] + destEdge.distToNode2;
    
    double finalDist = min(dist1, dist2);
    int closerDestNode = (dist1 < dist2) ? destEdge.node1 : destEdge.node2;
    
    if(finalDist == DBL_MAX) {
        cout << "No path exists" << endl;
        return 1;
    }
    
    cout << "Shortest distance: " << fixed << setprecision(3) << finalDist << " km" << endl;
    cout << endl;
    
    vector<int> path = getPath(closerDestNode);
