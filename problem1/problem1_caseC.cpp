...
int findNearestNode(Graph& G, double lon, double lat) {
    int nearestNode = -1;
    double minDist = DBL_MAX;
    
    for(int i = 0; i < G.V; i++) {
        double d = haversine(lon, lat, 
                            G.nodes[i].longitude, 
                            G.nodes[i].latitude);
        if(d < minDist) {
            minDist = d;
...
int main() {
    ...
    // case C: find nearest nodes for source and destination
    int srcNode = findNearestNode(G, src_lon, src_lat);
    int destNode = findNearestNode(G, dest_lon, dest_lat);
    
    double walkDistSrc = haversine(src_lon, src_lat,
                                   G.nodes[srcNode].longitude,
                                   G.nodes[srcNode].latitude);
    double walkDistDest = haversine(dest_lon, dest_lat,
                                    G.nodes[destNode].longitude,
                                    G.nodes[destNode].latitude);
    
    cout << "Source: (" << fixed << setprecision(6) << src_lon << ", " << src_lat << ")" << endl;
    cout << "Nearest node to source: " << srcNode 
         << " (" << G.nodes[srcNode].longitude << ", " << G.nodes[srcNode].latitude << ")" << endl;
    cout << "Walking distance to source node: " << fixed << setprecision(3) << walkDistSrc << " km" << endl;
    cout << endl;
    
    cout << "Destination: (" << fixed << setprecision(6) << dest_lon << ", " << dest_lat << ")" << endl;
    cout << "Nearest node to destination: " << destNode 
         << " (" << G.nodes[destNode].longitude << ", " << G.nodes[destNode].latitude << ")" << endl;
    cout << "Walking distance to destination node: " << fixed << setprecision(3) << walkDistDest << " km" << endl;
    cout << endl;
    ...
    double totalDistance = walkDistSrc + dist[destNode] + walkDistDest;
    
    cout << "Driving distance: " << fixed << setprecision(3) << dist[destNode] << " km" << endl;
    cout << "Total distance (including walking): " << fixed << setprecision(3) << totalDistance << " km" << endl;
    cout << endl;
