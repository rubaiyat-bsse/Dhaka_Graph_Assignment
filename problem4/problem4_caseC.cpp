...
int main(){
    ...
    if(srcNode == -1 || destNode == -1){
        cerr << "Error: Could not find nearest road nodes!" << endl;
        return 1;
    }
    
    double walkDistSrc = haversine(srcLon, srcLat, 
                                   G.nodes[srcNode].longitude, G.nodes[srcNode].latitude);
    double walkDistDest = haversine(G.nodes[destNode].longitude, G.nodes[destNode].latitude,
                                    destLon, destLat);
    
    double walkTimeSrc = (walkDistSrc / 2.0) * 60.0;
    double walkTimeDest = (walkDistDest / 2.0) * 60.0;
    
    cout << "Nearest road node to source: Node " << srcNode << " (walk " 
         << fixed << setprecision(3) << walkDistSrc << " km)" << endl;
    cout << "Nearest road node to destination: Node " << destNode << " (walk " 
         << walkDistDest << " km)" << endl;
    cout << endl;
    ...
    double drivingCost = dist[destNode];
    double totalCost = drivingCost;
    double finalArrivalTime = arrivalTime[destNode] + walkTimeDest;
    
    cout << "Minimum cost: ৳" << fixed << setprecision(2) << totalCost << endl;
    ...
    cout << "Walking distance: " << fixed << setprecision(3) << (walkDistSrc + walkDistDest) << " km" << endl;
    
    printSegmentedPath(G, path, adjustedStartTime, srcLon, srcLat, destLon, destLat);
    
    PathAnalysis analysis = detailedPrintPath(G, path);
    
    cout << "\n=== SUMMARY ===" << endl;
    cout << "Walking: " << fixed << setprecision(3) << (walkDistSrc + walkDistDest) 
         << " km (Cost: ৳0.00)" << endl;
