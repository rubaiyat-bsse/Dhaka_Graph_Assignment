...
void DijkstraFromEdge(Graph& G, EdgeMatch src, double startTimeMinutes) {
    ...
    // Calculate travel time to reach each edge endpoint
    double travelTimeToNode1 = (src.distToNode1 / 30.0) * 60.0; // minutes
    double travelTimeToNode2 = (src.distToNode2 / 30.0) * 60.0;
    
    dist[src.node1] = src.distToNode1 * 20.0;
    dist[src.node2] = src.distToNode2 * 20.0;
...
int main(){
    ...
    cout << "Source: (" << srcLon << ", " << srcLat << ")" << endl;
    cout << "Destination: (" << destLon << ", " << destLat << ")" << endl;
    cout << "Start time: " << formatTime(startTimeMinutes) << endl;
    cout << endl;
    
    // Find edges for source and destination
    EdgeMatch srcEdge = findEdgeForPoint(G, srcLon, srcLat);
    EdgeMatch destEdge = findEdgeForPoint(G, destLon, destLat);
    
    if(!srcEdge.found){
        cerr << "Error: Source not on any road edge!" << endl;
        return 1;
    }
    
    if(!destEdge.found){
        cerr << "Error: Destination not on any road edge!" << endl;
        return 1;
    }
    
    cout << "Source on edge: Node " << srcEdge.node1 << " - Node " << srcEdge.node2 << endl;
    cout << "Destination on edge: Node " << destEdge.node1 << " - Node " << destEdge.node2 << endl;
    cout << endl;
    
    initGlobalArray(G.V);
    DijkstraFromEdge(G, srcEdge, startTimeMinutes);
    
    // Find minimum cost to either destination endpoint
    double cost1 = dist[destEdge.node1] + destEdge.distToNode1 * 20.0;
    double cost2 = dist[destEdge.node2] + destEdge.distToNode2 * 20.0;
    
    double travelTime1 = (destEdge.distToNode1 / 30.0) * 60.0;
    double travelTime2 = (destEdge.distToNode2 / 30.0) * 60.0;
    
    double arrivalTime1 = arrivalTime[destEdge.node1] + travelTime1;
    double arrivalTime2 = arrivalTime[destEdge.node2] + travelTime2;
    
    int destNode;
    double finalCost, finalArrivalTime;
    
    if(cost1 <= cost2){
        destNode = destEdge.node1;
        finalCost = cost1;
        finalArrivalTime = arrivalTime1;
    } else {
        destNode = destEdge.node2;
        finalCost = cost2;
        finalArrivalTime = arrivalTime2;
    }
    ...
    cout << "Minimum cost: ৳" << fixed << setprecision(2) << finalCost << endl;
    cout << "Arrival time: " << formatTime(finalArrivalTime) << endl;
    double totalTimeMinutes = finalArrivalTime - startTimeMinutes;
